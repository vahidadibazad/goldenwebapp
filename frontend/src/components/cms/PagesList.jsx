// frontend/src/components/cms/PagesList.jsx
...
function PagesList() {
  ...
  const fetchData = useCallback(async (page = 1, pageSize = 10) => {
    setLoading(true);
    try {
      // دریافت ContentType برای Pages
      const typesRes = await cmsService.getContentTypes();
      const pageType = typesRes.data.data.find(t => t.apiName === 'pages');
  ...
  const handleDelete = async (id) => {
    try {
      ...
      message.success('صفحه با موفقیت حذف شد');
      fetchData();
    } catch (error) {
  ...
  const handleArchive = async (id) => {
    try {
      ...
      message.success('صفحه با موفقیت بایگانی شد');
      fetchData();
    } catch (error) {
  ...
  const clearFilters = () => {
    setFilters({ status: '', locale: '', dateRange: [] });
    setSearch('');
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (locale) => {
        ...
        return <Tag>{map[locale] || locale}</Tag>;
      },
      filters: [
        { text: 'فارسی', value: 'fa' },
      ...
      onFilter: (value, record) => record.locale === value,
    },
    {
      title: 'بازدید',
      dataIndex: 'viewCount',
        ...
        return (
          <Space size={isPhone ? 2 : 4}>
            <Tooltip title="مشاهده" placement="top">
              <Link to={`/pages/${record.slug}`} target="_blank">
                <Button
                  type="text"
                  icon={<EyeOutlined />}
            ...
            <Tooltip title="ویرایش" placement="top">
              <Link to={`/cms/pages/edit/${record._id}`}>
                <Button
                  type="text"
                  icon={<EditOutlined />}
                  size={isPhone ? 'small' : 'middle'}
            ...
            {status === 'draft' && (
              <Tooltip title="انتشار" placement="top">
                <Popconfirm
                  title="آیا از انتشار این صفحه اطمینان دارید؟"
            ...
            {status === 'published' && (
              <Tooltip title="بایگانی" placement="top">
                <Popconfirm
                  title="آیا از بایگانی این صفحه اطمینان دارید؟"
            ...
            {(status === 'draft' || status === 'archived') && (
              <Tooltip title="حذف" placement="top">
                <Popconfirm
                  title="آیا از حذف این صفحه اطمینان دارید؟"
      ...
      >
        ...
        <Space wrap size={[4, 4]}>
          ...
          <Link to="/cms/pages/new">
            ...
            >
              {isPhone ? 'جدید' : isMobile ? 'افزودن' : 'صفحه جدید'}
            </Button>
          </Link>
        </Space>
      </div>
      ...
      >
        ...
        {showFilters && (
          ...
          >
            ...
            <RangePicker
              ...
              format="YYYY/MM/DD"
            />
            {(filters.status || filters.locale || filters.dateRange.length > 0) && (
              <Button
                size="small"
                onClick={() =>
                  setFilters({
        ...
        <Table
          ...
          pagination={{
            ...
            showTotal: (total) => (isPhone ? `${total}` : `تعداد ${total} صفحه`),
            ...
            emptyText: (
              <div style={{ padding: isPhone ? '30px 0' : '60px 0' }}>
                ...
                <Link to="/cms/pages/new">
                  ...
                  >
                    {isPhone ? 'اولین صفحه' : 'ایجاد اولین صفحه'}
                  </Button>
                </Link>
              </div>
            ),
